...
const engine_scoringEngine = require("../engine/scoring-engine.js");
const engine_rotationEngine = require("../engine/rotation-engine.js");
const models_match = require("../models/match.js");
const services_syncService = require("../services/sync-service.js");
const useMatchStore = common_vendor.defineStore("match", () => {
  ...
  const set3Swapped = common_vendor.ref(false);
  const timeoutsA = common_vendor.ref([0, 0, 0]);
  const timeoutsB = common_vendor.ref([0, 0, 0]);
  const lastTimeout = common_vendor.ref(null);
  const pendingSet3Swap = common_vendor.ref(null);
  const subsA = common_vendor.ref([0, 0, 0]);
  const subsB = common_vendor.ref([0, 0, 0]);
  ...
  function startMatch(teamAConfig, teamBConfig, initialServingTeam, teamABenchPlayers, teamBBenchPlayers, setCaps) {
    match.value = models_match.createMatch(teamAConfig, teamBConfig, initialServingTeam);
    match.value.status = "live";
    scoringEngine.value = new engine_scoringEngine.ScoringEngine(initialServingTeam, setCaps);
    rotationEngine.value = new engine_rotationEngine.RotationEngine(teamAConfig.startingPositions, teamBConfig.startingPositions);
  ...
  function checkSet3Swap() {
    if (!scoringEngine.value)
      return false;
    if (scoringEngine.value.currentSet !== 3 || set3Swapped.value)
      return false;
  ...
  function loadSnapshot(snapshot) {
    const engine = new engine_scoringEngine.ScoringEngine(snapshot.scoringState.servingTeam, snapshot.scoringState.setCaps);
    engine.setState(snapshot.scoringState);
    scoringEngine.value = engine;
    const rotEngine = new engine_rotationEngine.RotationEngine(
...
exports.useMatchStore = useMatchStore;
