...
const setResults = computed(() => indoorMatchStore.match?.setScores ?? [])
const winnerName = computed(() => {
  if (!indoorMatchStore.match?.winner) return ''
...
<template>
  <view class="page">
    <view class="result-card">
      ...
      <text class="result-sets-score">{{ indoorMatchStore.setsWonA }} : {{ indoorMatchStore.setsWonB }}</text>
...
.team-a-name { color: $indoor-team-a; }
.team-b-name { color: $indoor-team-b; }
...
.set-row {
  ...
  &.a-win { background-color: rgba($indoor-team-a, 0.1); }
  &.b-win { background-color: rgba($indoor-team-b, 0.1); }
}
