...
const setResults = computed(() => matchStore.match?.setScores ?? [])
const winnerName = computed(() => {
  if (!matchStore.match?.winner) return ''
...
<template>
  <view class="page">
    <view class="result-card">
      ...
      <text class="result-sets-score">{{ matchStore.setsWonA }} : {{ matchStore.setsWonB }}</text>
...
.set-row {
  ...
  &.a-win { background-color: rgba($team-a-color, 0.1); }
  &.b-win { background-color: rgba($team-b-color, 0.1); }
}
...
.set-winner {
  ...
  .a-win & { color: $team-a-color; font-weight: bold; }
  .b-win & { color: $team-b-color; font-weight: bold; }
}
