...
const selectedPosIndex = ref(-1)  // 0-4 对应1-5号位
...
const canSubstitute = computed(() => {
  ...
  const otherNumbers = rotation
    ...
    .map(pid => matchStore.getPlayer(pid))
    .filter(Boolean)
    .map(p => p!.number)
...
function doSubstitute() {
  ...
  const bench = matchStore.getBench(selectedTeam.value)
  ...
  const result = matchStore.substitute(selectedTeam.value, playerOutId, playerIn.id, playerIn)
  if (result.success) {
...
function showCaptainSelectDialog(team: TeamSide) {
  ...
  const players = rotation.map(pid => matchStore.getPlayer(pid)).filter(Boolean) as Player[]
...
<template>
  <view class="page" v-if="matchStore.isLive">
    ...
    <view class="card section">
      ...
      <view class="player-grid">
        ...
        >
          <text class="pos-tag">{{ item.position }}号位</text>
  ...
  <!-- 底部固定按钮 -->
  <view v-if="matchStore.isLive" class="fixed-bottom">
